...
namespace VirtualClient.Dependencies
{
    ...
    /// <summary>
    /// Installation component for a specific AMD GPU Driver
    /// </summary>
    public class AMDSpecificGPUDriverInstallation : VirtualClientComponent
    {
        ...
        /// <summary>
        /// Initializes a new instance of the <see cref="AMDSpecificGPUDriverInstallation"/> class.
        /// </summary>
        /// <param name="dependencies">An enumeration of dependencies that can be used for dependency injection.</param>
        /// <param name="parameters">A series of key value pairs that dictate runtime execution.</param>
        public AMDSpecificGPUDriverInstallation(IServiceCollection dependencies, IDictionary<string, IConvertible> parameters)
            ...
            get
            {
                switch (this.Platform)
                {
                    case PlatformID.Win32NT:
                        return this.Parameters.GetValue<bool>(nameof(AMDSpecificGPUDriverInstallation.RebootRequired), false);
                    default:
                        return this.Parameters.GetValue<bool>(nameof(AMDSpecificGPUDriverInstallation.RebootRequired), true);
                }
            }
        }
        ...
        public string Username
        {
            get
            {
                string username = this.Parameters.GetValue<string>(nameof(AMDSpecificGPUDriverInstallation.Username), string.Empty);
        ...
        public string LinuxInstallationFile
        {
            get
            {
                return this.Parameters.GetValue<string>(nameof(AMDSpecificGPUDriverInstallation.LinuxInstallationFile), string.Empty);
            }

            set
            {
                this.Parameters[nameof(AMDSpecificGPUDriverInstallation.LinuxInstallationFile)] = value;
            }
        }
        ...
        protected override Task ExecuteAsync(EventContext telemetryContext, CancellationToken cancellationToken)
        {
            ...
            {
                State installationState = await this.stateManager.GetStateAsync<State>(nameof(AMDSpecificGPUDriverInstallation), cancellationToken)
                ...
                if (installationState == null)
                {
                    if (this.Platform == PlatformID.Win32NT)
                    {
                        ...
                        await this.stateManager.SaveStateAsync(nameof(AMDSpecificGPUDriverInstallation), new State(), cancellationToken)
                            .ConfigureAwait(false);
        ...
        private async Task InstallAMDGPUDriverLinux(EventContext telemetryContext, CancellationToken cancellationToken)
        {
            ...
            foreach (var commandsList in commandsLists)
            {
                foreach (string command in commandsList)
                {
                    ...
                    {
                        await this.LogProcessDetailsAsync(process, telemetryContext, "AMDSpecificGPUDriverInstallation", logToFile: true)
                            .ConfigureAwait(false);
        ...
        private async Task InstallAMDGPUDriverWindows(EventContext telemetryContext, CancellationToken cancellationToken)
        {
            ...
            string? installationFile = this.fileSystem.Directory.GetFiles(amdDriverInstallerPackage.Path, "*.exe").FirstOrDefault();
